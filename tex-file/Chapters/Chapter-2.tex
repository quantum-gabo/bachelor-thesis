...
The simplest---and remarkably effective---approximation for the exchange-correlation functional is the local-density approximation (LDA)\supercite{Kohn1999}. It approximates the local electron density in a inhomogeneous system by an homogeneous electron gas (HEG) having the same electron density. 
\begin{equation}
  \label{eq58}
...
\begin{equation}
  ...
  \end{cases}
\end{equation}
LDA has demonstrated remarkable success for most applications involving systems with slowly varying densities or systems with high densities. Nevertheless, LDA---and LSDA, the spin-polarised version of LDA---fail in systems, like heavy fermion systems, so dominated by strong electron-electron interactions that it loses any resemblance to non-interacting electron gases.
