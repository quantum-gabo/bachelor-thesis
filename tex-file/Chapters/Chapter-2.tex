...
Finally, AIMD simulations may also include temperature control, 
...
To this end, we shall introduce the concept of pseudopotentials\supercite{Hellmann1935}.
...
Various pseudopotentials have been proposed to achieve the desired 
...
The reciprocal space is divided into regions called Brillouin zones. 
The first Brillouin zone is defined as the region in reciprocal space closest
to the origin ($\Gamma$ point). It plays a crucial role in band structure and density of 
states calculations, as it contains all the unique wave vectors $\mathbf{k}$ that 
describe the electronic states of the crystal. Moreover, 
the volume of the first Brillouin zone is given by
\begin{equation}
    \label{eq87}
...
Finally---because electrons in a crystal are subjected to a periodic potential
---single-electron wavefunctions follow Bloch's theorem, and can be expressed as 
\begin{equation}
    \label{eq88}
    \psi_{\mathbf{k}} = e^{i\mathbf{k} \cdot \mathbf{r}} u_{\mathbf{k}}(\mathbf{r})
\end{equation}
where $\mathbf{k}$ is the wave vector inside the first Brillouin zone, and 
...
Equation \ref{eq83} is utilised in this work to report the aforementioned thermodynamical
...
% One way to hugely speed up AIMD calculations is by using force fields---
% parameterised models of the interatomic potential.
%  However making accurate classical force fields manually is a rather time-consuming task and often 
% requires a tremendous expertise and know-how. 
...
The force field is then generated during AIMD simulations following 
the steps below:
\begin{enumerate}
...
DFTB+ is a versatile open-sourse quantum mechanical simulation code 
that uses
